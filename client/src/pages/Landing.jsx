...
export default function Landing() {
  return (
    <div className="min-h-screen bg-slate-100">
      <header className="sticky top-0 bg-white/80 backdrop-blur-lg border-b border-slate-300 px-6 sm:px-8 py-4 flex items-center justify-between z-10">
      ...
      <main className="px-6 sm:px-8 py-20 sm:py-24 text-center">
        ...
        <p className="text-base sm:text-lg text-slate-600 mb-8 max-w-3xl mx-auto">
          A knowledge base and assistant for clinical and support staff — find
          the right procedure, or just ask.
        </p>
        <Link
          to="/login"
      ...
      <section className="px-6 sm:px-8 pb-24 sm:pb-32">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {FEATURES.map((f) => (
            <div
              key={f.title}
              className="border border-slate-200 rounded-xl p-6 transition hover:bg-white hover:shadow-lg"
            >
