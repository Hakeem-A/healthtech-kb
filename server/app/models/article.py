...
class Article(Base):
    ...
    )  # Let's make it nullable in case author is deleted, or RESTRICT depending on preferences. Nullable with SET NULL is safer.
    status = Column(String, default="draft")  # draft, under_review, published, archived
    tags = Column(String, nullable=True)
    rejection_reason = Column(
        Text, nullable=True
    )  # Varchar column for easy display/fallback
