...
def get_analytics_summary(db: Session = Depends(get_db)):
    ...
    published_count = (
        db.query(func.count(Article.id)).filter(Article.status == "published").scalar()
    ) or 0

    total_views = db.query(func.sum(Article.views)).scalar() or 0

    avg_rating, rating_count = db.query(
        func.avg(Feedback.rating), func.count(Feedback.id)
    ).first() or (None, 0)
    ...
    return AnalyticsSummary(
        ...
        top_articles=[
            TopArticle(id=a.id, title=a.title, views=a.views) for a in top_articles
        ],
